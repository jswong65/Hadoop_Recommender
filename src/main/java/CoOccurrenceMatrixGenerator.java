...
public class CoOccurrenceMatrixGenerator {
	public static class MatrixGeneratorMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			//value = userid \t movieA=rating, movieB=rating...
			//key = movie1: movie2 value = 1
			//calculate each user rating list: <movieA, movieB>

			String[] tmp = value.toString().trim().split("\t");
			if(tmp.length < 2) {
				// the user haven't rated any movie yet.
				return;
			}
			String[] movie_rating = tmp[1].toString().split(",");
			String movie1, movie2;

			for(int i=0; i<movie_rating.length; i++){
				movie1 = movie_rating[i].split("=")[0];

				for(int j=0; j<movie_rating.length; j++){
					movie2 = movie_rating[j].split("=")[0];
					outKey.set(movie1 + ";" + movie2);
